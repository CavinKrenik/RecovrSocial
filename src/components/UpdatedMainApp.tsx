...
import CleanTimeTracker from './CleanTimeTracker';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-teal-800">
      ...
      <main className="pb-20">
        {activeTab === 'home' && (
          <div className="text-center py-12 px-4">
            ...
            <div className="space-y-3 text-gray-300">
              <p className="text-lg font-medium">Explore the Events Page for local Events, or add one!</p>
              <p className="text-lg font-medium">Also Track Your Clean Date.</p>
            </div>
          </div>
        )}
        
        {activeTab === 'resources' && profilePage === 'clean-tracker' && (
          <CleanTimeTracker onBack={() => setProfilePage('main')} />
        )}
        
        {activeTab === 'resources' && profilePage === 'main' && (
          <UpdatedResourcesSection onNavigateToTracker={() => setProfilePage('clean-tracker')} />
        )}
        ...
        {activeTab === 'profile' && profilePage !== 'main' && profilePage !== 'journal' && (
          <ProfilePages
...
const ProfileSection: React.FC<{ userNickname: string; onNavigate: (page: ProfilePage) => void }> = ({ userNickname, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    // Listen for clean date updates
    const handleCleanDateUpdate = () => {
      updateDaysClean();
    };
    
    window.addEventListener('cleanDateUpdated', handleCleanDateUpdate);
    
    return () => {
      window.removeEventListener('cleanDateUpdated', handleCleanDateUpdate);
    };
  }, []);
      ...
      <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
        ...
        <div className="space-y-3">
          ...
          <button onClick={() => onNavigate('set-clean-date')} className="w-full bg-white/10 hover:bg-white/20 text-white py-3 px-4 rounded-lg text-left transition-colors">
            {daysClean > 0 ? 'Update Clean Date' : 'Set Clean Date'}
          </button>
